...
import com.moa.moakotlin.ui.voice.AGEventHandler
import io.agora.rtc.IRtcEngineEventHandler
import io.agora.rtc.RtcEngine
import java.util.concurrent.ConcurrentHashMap

class MyEngineEventHandler(private val mContext: Context, private val mConfig: EngineConfig) {

    private val mEventHandlerList: ConcurrentHashMap<AGEventHandler, Int> =
        ConcurrentHashMap<AGEventHandler, Int>()
...
//            log.debug("onUserOffline " + (uid and 0xFFFFFFFFL) + " " + reason)

            // FIXME this callback may return times
            val it: Iterator<AGEventHandler> = mEventHandlerList.keys.iterator()
            while (it.hasNext()) {
                val handler: AGEventHandler = it.next()
...
//            log.debug("onConnectionLost")
            ...
            while (it.hasNext()) {
                ...
                handler.onExtraCallback(
                    ...
                    ConstantApp.AppError.NO_NETWORK_CONNECTION
                )
            }
        }
...
//            log.debug("onJoinChannelSuccess " + channel + " " + (uid and 0xFFFFFFFFL) + " " + elapsed)
            mConfig.mUid = uid
            val it: Iterator<AGEventHandler> = mEventHandlerList.keys.iterator()
            while (it.hasNext()) {
                val handler: AGEventHandler = it.next()
...
//            log.debug("onRejoinChannelSuccess " + channel + " " + (uid and 0xFFFFFFFFL) + " " + elapsed)
        }
